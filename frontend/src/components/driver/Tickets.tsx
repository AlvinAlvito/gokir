...
export default function DriverTickets() {
  ...
  const fetchAll = async () => {
    try {
      ...
      const [r1, r2] = await Promise.all([
        ...
        fetch(`${API_URL}/driver/tickets/orders`, { credentials: "include" }),
      ]);
  ...
  const handleOrder = async (e?: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setMsg(null);
      const r = await fetch(`${API_URL}/driver/tickets/order`, {
        method: "POST",
        credentials: "include",
    ...
    <div className="p-5 rounded-3xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] lg:p-6 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Saldo tiket untuk transaksi antar/jemput.</p>
        </div>
