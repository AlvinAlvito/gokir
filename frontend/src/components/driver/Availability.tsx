...
export default function DriverAvailability() {
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/driver/availability`, {
        method: "PATCH",
        credentials: "include",
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            <h4 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              Informasi Ketersediaan Anda
            </h4>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Atur status aktif, wilayah, dan pantau lokasi terkini.
            </p>
            {data?.updatedAt && (
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[720px] m-4">
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-10 space-y-4">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Update Ketersediaan</h4>
            <p className="text-sm text-gray-500 dark:text-gray-400">Ubah status dan wilayah. Lokasi diisi otomatis dan tidak bisa diedit.</p>
          </div>
          ...
          <form className="grid grid-cols-1 lg:grid-cols-2 gap-6" onSubmit={handleSave}>
            ...
            <div className="space-y-2 lg:col-span-2">
              ...
              <Input value={note} onChange={(e: any) => setNote(e.target.value)} placeholder="Mis. standby gerbang utara" />
            </div>
