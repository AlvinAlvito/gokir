...
type Order = {
  ...
  driver?: { id: string; username?: string | null; email?: string | null; phone?: string | null } | null;
...
export default function StoreOrdersHistoryPage() {
  ...
  return (
    <>
      <PageMeta title="Riwayat Transaksi" description="Daftar transaksi toko" />
...
const OrderCard = ({ order }: OrderCardProps) => {
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/store/orders/${order.id}/report`, {
        method: "POST",
        credentials: "include",
        body: fd,
      });
        ...
        <div className="space-y-1">
          ...
          {order.customer && (
            <div className="text-xs text-gray-500 dark:text-gray-400">
              <p>Customer: {order.customer.username || "Customer"}</p>
              {order.customer.phone && <p>WA: {order.customer.phone}</p>}
